...
`## ---` の行と後続の空行を削除し、次の行を `##` 見出しとして結合します。Google ドキュメントの Markdown 変換で章の区切りに挿入される `## ---` を整形するための機能です。
...
**第2章 Vision Transformer の基礎**

# 変換後
## **第2章 Vision Transformer の基礎**
```

### 機能 2：Imageタグ → LaTeX 変換
...
`[imageX]: <data:image/...>` 形式の行を削除します。機能 2 と組み合わせて使用することを想定していますが、単独でも動作します。
